...
def find_cart(tenant_ctx: TenantContext) -> Cart | None:
    if tenant_ctx.user_id:
        return Cart.objects.filter(store_id=tenant_ctx.store_id, user_id=tenant_ctx.user_id).first()
    if not tenant_ctx.session_key:
        return None
    return Cart.objects.filter(store_id=tenant_ctx.store_id, session_key=tenant_ctx.session_key).first()
